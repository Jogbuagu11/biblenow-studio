-- Fix the livestream_weekly_usage table and trigger system
-- Drop existing functions first to avoid conflicts

-- 1. Drop existing functions if they exist
DROP FUNCTION IF EXISTS get_user_streaming_limit(uuid);
DROP FUNCTION IF EXISTS get_weekly_streaming_usage(uuid);
DROP FUNCTION IF EXISTS get_or_create_weekly_usage(uuid, date);
DROP FUNCTION IF EXISTS add_streaming_minutes(uuid, integer, date);
DROP FUNCTION IF EXISTS notify_streaming_limit_status();

-- 2. Create the livestream_weekly_usage table
...
-- 4. Create function to get or create weekly usage record
...
-- 5. Create function to add streaming minutes
...
-- 6. Create function to get user's streaming limit
...
-- 7. Create function to get current week's usage
...
-- 8. Create the trigger function for streaming limit notifications
