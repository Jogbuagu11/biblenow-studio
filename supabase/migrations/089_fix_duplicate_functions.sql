-- Fix duplicate function issue that prevents streams from ending
-- This addresses the "function get_weekly_streaming_usage(uuid) is not unique" error
...
-- Step 2: Drop all triggers (with IF EXISTS to avoid errors)
...
-- Step 4: Create indexes (with IF NOT EXISTS to avoid errors)
CREATE INDEX IF NOT EXISTS idx_livestream_weekly_usage_user_id ON livestream_weekly_usage(user_id);
CREATE INDEX IF NOT EXISTS idx_livestream_weekly_usage_week_start ON livestream_weekly_usage(week_start_date);
CREATE INDEX IF NOT EXISTS idx_livestream_weekly_usage_user_week ON livestream_weekly_usage(user_id, week_start_date);

-- Step 5: Enable RLS
ALTER TABLE livestream_weekly_usage ENABLE ROW LEVEL SECURITY;

-- Step 6: Drop existing policies first, then create new ones
DROP POLICY IF EXISTS "Users can view their own weekly usage" ON livestream_weekly_usage;
DROP POLICY IF EXISTS "Users can insert their own weekly usage" ON livestream_weekly_usage;
DROP POLICY IF EXISTS "Users can update their own weekly usage" ON livestream_weekly_usage;
DROP POLICY IF EXISTS "Service role can do everything" ON livestream_weekly_usage;
...
-- Step 7: Create the functions with unique signatures
