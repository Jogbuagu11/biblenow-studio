-- Fix the check_weekly_streaming_limit function to be completely dynamic
...
WITH test_cases AS (
    SELECT 
        ...
        sp.name as plan_name,
        sp.streaming_minutes_limit as configured_limit
    FROM public.verified_profiles vp
    LEFT JOIN public.subscription_plans sp ON vp.subscription_plan_id = sp.id
    -- Get a sample of users from different plans
    WHERE vp.id IN (
        SELECT DISTINCT vp2.id 
        FROM public.verified_profiles vp2
        LEFT JOIN public.subscription_plans sp2 ON vp2.subscription_plan_id = sp2.id
        GROUP BY vp2.id, sp2.streaming_minutes_limit
        HAVING sp2.streaming_minutes_limit IS DISTINCT FROM LAG(sp2.streaming_minutes_limit) 
            OVER (ORDER BY sp2.streaming_minutes_limit)
        LIMIT 5
    )
)
SELECT 
    tc.email,
    tc.subscription_plan,
    tc.plan_name,
    tc.configured_limit as plan_limit_configured,
    cwsl.limit_minutes as limit_returned,
    cwsl.current_minutes,
    cwsl.remaining_minutes,
    cwsl.usage_percentage,
    CASE 
        WHEN tc.configured_limit = cwsl.limit_minutes THEN 'OK'
        ELSE 'MISMATCH'
    END as limit_check
FROM test_cases tc
CROSS JOIN LATERAL check_weekly_streaming_limit(tc.user_id) cwsl;
