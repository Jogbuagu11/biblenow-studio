-- Update the trigger function with better edge function logging
...
BEGIN
    ...
    -- Log the settings
    RAISE NOTICE 'Settings loaded - URL: %, Key exists: %', 
        supabase_url, 
        CASE WHEN service_role_key IS NOT NULL THEN 'true' ELSE 'false' END;
    ...
    RAISE NOTICE 'Streaming limit check - User: %, Email: %, Usage: %/% minutes (%%), Next Reset: %', 
        NEW.user_id,
        user_profile.email, 
        current_usage,
        user_limit,
    ...
    IF usage_percentage >= 75 AND usage_percentage < 100 THEN
        ...
        RAISE NOTICE 'Attempting to send warning email for user: % (%%)', 
        ...
        INSERT INTO public.studio_notifications (
            ...
            jsonb_build_object(
                ...
                'remaining_minutes', user_limit - current_usage,
                'reset_date', next_week_start,
                'edge_function_status', http_response.status,
                'edge_function_response', http_response.content
            ),
            NOW()
        );
    END IF;

    -- Similar logging for 100% threshold...
    IF usage_percentage >= 100 THEN
        -- Log attempt to send limit reached email
        RAISE NOTICE 'Attempting to send limit reached email for user: %', NEW.user_id;
